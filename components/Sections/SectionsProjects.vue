    ...
    <div class="relative z-10">
      ...
      <div class="px-6 lg:px-16 mb-12 lg:mb-16">
        <div class="flex items-center gap-4 mb-6">
          ...
          <span class="font-mono text-xs text-base-400 tracking-wider uppercase">Selected Work</span>
        </div>
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          ...
          <div class="flex items-center gap-4">
            <button 
              @click="scroll('left')"
              class="w-12 h-12 border border-base-600/50 flex items-center justify-center text-base-300 hover:border-coral-500 hover:text-coral-500 transition-colors"
      ...
      >
        ...
        >
          ...
          >
            ...
            <!-- Overlay -->
            <div class="absolute inset-0 bg-gradient-to-t from-surface via-surface/20 to-transparent opacity-60 group-hover:opacity-80 transition-opacity"></div>
            
            <!-- Content overlay -->
            <div class="absolute bottom-0 left-0 right-0 p-6">
              <div class="flex items-center gap-3 mb-2">
                <span class="font-mono text-xs text-coral-500">{{ project.year }}</span>
                <span class="w-8 h-px bg-base-600"></span>
                <span class="text-xs text-base-400">{{ project.category }}</span>
              </div>
              <h3 class="font-display font-bold text-2xl lg:text-3xl text-base-50 group-hover:text-coral-500 transition-colors">
                {{ project.name }}
              </h3>
            </div>

            <!-- Hover arrow -->
            <div class="absolute top-6 right-6 w-10 h-10 border border-base-50/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <Icon name="tabler:arrow-up-right" class="w-4 h-4 text-base-50" />
            </div>
          </a>
          
          <!-- Impact text below image -->
          <p class="text-sm text-base-400 pl-2 border-l border-coral-500/30">
            {{ project.impact }}
          </p>
        </div>
